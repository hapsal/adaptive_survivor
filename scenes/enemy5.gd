...
func take_damage(damage_amount):
	...
	if health > 0:
		...
		hurt_tween.tween_property(%HumanEnemy, "modulate", Color(3, 0.25, 0.25), 0.2)
		hurt_tween.chain().tween_property(%HumanEnemy, "modulate", Color(1, 1, 1), 0.2)
	else:
